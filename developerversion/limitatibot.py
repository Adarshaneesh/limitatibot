...
#Versione 1.0.1 Limitati Bot S 24/04/18 

""" Limitati Bot S is a simple bot for telegram that allows limitate users to send you a message.
...
def filtro(msg): #Filtro contro flood ed utenti bannati
    ...
    banned = scarica(arcbanned,[]) #Scarica l'archivio degli utenti bannati
    ...
    if chat_id == amministratore:
        if command == "/ban": #Commando ban
            if destinatario != 0 and destinatario != amministratore:
                ...
                carica(arcbanned,banned) #Aggiorna il file con la lista dei bannati
        elif command == "/unban":
            if destinatario != 0 and destinatario != amministratore:
                banned.remove(destinatario)
                carica(arcbanned,banned)
